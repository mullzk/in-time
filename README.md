...
- `/takt`, `/kaskade`, `/zeitkarte`: simple html-pages loading js and css and
...
- _What the views draw is computed in the browser_, from the schedule blobs
  ...
  - `ConnectionList` (every leg of the day in departure order, one shared
    ...
    station) yield the reachability tree the kaskade and zeitkarte views draw —
    ~2.3 million legs in milliseconds.
