...
- The pure, unit-tested core — **`Camera`** (LV95 world space, screen↔world,
  zoom/pan clamps), **`TimeModel`** (operating window, tempo, loop, scrubber),
  **`VehiclePositionEngine`** (reads a binary schedule blob and answers
  `activeAt(t)` with interpolated positions) and **`StationCatalog`** (the
  merged stations of both networks, with the search ranking) — carries no p5 or
  DOM, so `node:test` covers it directly. Rendering is verified visually.
