...
from pipeline.artifacts import STATIONS_RAIL_NAME
...
def test_stations_rail_bytes_returns_file_content(tmp_path: Path) -> None:
    payload = json.dumps([{"didok": 8_507_000, "name": "Bern"}]).encode("utf-8")
    data_dir = _publish(tmp_path, date(2026, 7, 16), payload)

    assert PublishedSchedule(data_dir).stations_rail_bytes() == payload


def test_stations_rail_bytes_is_none_when_missing(tmp_path: Path) -> None:
    data_dir = _publish(tmp_path, date(2026, 7, 16), None)

    assert PublishedSchedule(data_dir).stations_rail_bytes() is None
